...
class Tester:
    ...
    def running(self, nums_to_generate, output_dir, generated_folder_name):
        ...
        * 테스트 셋팅 및 진행
        ...
        mean = self.image_pyramids[-1][const_var_singan.KEY_IMGPYR_MEAN]
        std = self.image_pyramids[-1][const_var_singan.KEY_IMGPYR_STD]
        ...
        for num, fake_x in enumerate(iterable=fake_x_batch, start=1):
            ...
            # 결과물 이미지 저장
            generated_img_dir = UtilLib.getNewPath(path=output_dir, add=const_var_singan.OUTPUT_DIR_SUFFIX_GENERATED_IMG.format(generated_folder_name))
            generated_img_filepath = UtilLib.getNewPath(path=generated_img_dir, add=const_var_singan.GENERATED_IMG_FILE_NAME.format(num))
    ...
    def _test(self):
        ...
        * 테스트 진행
        ...
        # 관련 변수 설정
        x = self.image_pyramids[highest_scale_num][const_var_singan.KEY_IMGPYR_IMAGE]
